# sft.py
"""Supervised Fine-Tuning for the 672M LLM - V3"""
...
class SFTConfig:
    ...
    base_checkpoint: str = "/workspace/checkpoints_v3_700m_xsa_30b/step_028610.pt"
    
    # Output
    checkpoint_dir: str = "/workspace/checkpoints_sft_v3"
...
def sft_train(config: SFTConfig):
    ...
    # Initialize wandb
    if HAS_WANDB:
        wandb.init(project="llm-672m", name="sft_v3", config=vars(config))
    ...
    assert 671_000_000 < n_params < 673_000_000, (
        ...
        f"Check architecture config (d_model/n_heads/n_kv_heads/use_xsa set right?)."
    )
   ...
   # Restore optimizer state if resuming
    ...
    # Final save
    path = os.path.join(config.checkpoint_dir, "sft_final_v3.pt")  # rename so it doesn't clobber V1 sft_final
    torch.save({
        "model_weights": model.state_dict(),
        "step": total_steps,
